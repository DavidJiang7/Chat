...
namespace Chat
{
    ...
    public class ClientHub : Hub
    {
        ...
        public void Send(string name, string message)
        {
            ...
            var user = ConnectedUsers.FirstOrDefault(it => it.FromConnectionId == id);
            if (user != null)
            {
                Clients.Client(user.ToConnectionId).addNewMessageToPage(name, message);
            }
        }
        ...
        public void Connect(int userID, string userName)
        {
            ...
            lock (ConnectedUsers)
            {
                ...
                //if (ConnectedUsers.Count(it => it.ConnectionId == id) == 0)
                {
                    ...
                    Models.User service = GetRandService();
        ...
        public Models.User GetRandService()
        {
            if (ServiceHub.ConnectedUsers != null && ServiceHub.ConnectedUsers.Count > 0)
            {
                Models.User u = ServiceHub.ConnectedUsers[Utils.Rand.Int(0, ServiceHub.ConnectedUsers.Count)];
                Clients.Client(u.FromConnectionId).addNewClientTagToPage();
                return u;
            }
            return null;
        }
